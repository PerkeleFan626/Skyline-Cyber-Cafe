...
public:
    ...
    string hist_f = filePath::HISTORY_FILE
    );
    ...
    vector<UserSession> get_all_sessions();
    bool remove_active_session(int userId);

    // --- Transaction History ---
    bool add_transaction_history(const TransactionRecord& record);
};